...
void main() {
    ...
    float downDistance = -u_cameraPosition.z + pos.z;
    float xDistance = -u_cameraPosition.z + pos.z;
    float curveStart = 15.0;
    if (downDistance > curveStart){
        downDistance = (downDistance - curveStart) * 0.04;
        xDistance = (xDistance-curveStart) * u_turnAmount;
    }
    else {
        downDistance = 0.0;
        xDistance = 0.0;
    }
    gl_Position = u_projViewTrans * (pos + vec4(sign(xDistance) * xDistance*xDistance, -downDistance*downDistance, 0, 0));
    ...
    #ifdef fogFlag
    float flen = - u_cameraPosition.z + pos.z;
